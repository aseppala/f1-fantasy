# 2026 Australian GP - Fantasy Team Pick #3 ("Ferrari Nuclear")

**Budget:** $100M | **Deadline:** Before qualifying, Saturday March 7
**Strategy:** HIGH RISK, HIGH REWARD. Triple Ferrari + 2x driver.

---

## The Bet

Ferrari goes 1-2 in Australia. You own BOTH drivers AND the constructor. 2x driver on Leclerc. If it hits, you blow the doors off the friends league. If it doesn't, you crash and burn.

---

## Team

### Drivers (5)

| # | Driver | Team | Price | Rationale |
|---|--------|------|-------|-----------|
| 1 | **Charles Leclerc** | Ferrari | $22.8M | **2x DRIVER.** Fastest in all testing (1:31.992). P1 FP1. P5 FP2. $4.9M cheaper than Verstappen. If he wins with 2x: 60-80 fantasy pts from ONE driver. |
| 2 | **Lewis Hamilton** | Ferrari | $22.5M | P2 FP1, P4 FP2. 4th favorite in betting odds (7/1). Cheaper than Leclerc by $0.3M. "Determined to rebound" from 16.3 pts/race avg in 2025. |
| 3 | **Oliver Bearman** | Haas | $7.4M | Proven value king. 57% owned. Overtaking machine. Needs 9 pts for price rise. |
| 4 | **Arvid Lindblad** | Racing Bulls | ~$5.5M | P5 FP1, P8 FP2 as a rookie. Best PPM potential on grid at this price. |
| 5 | **Valtteri Bottas** | Cadillac | $5.9M | Cheapest driver. Grid penalty has been lifted - no penalty for Australia. Pure budget enabler. |

**Driver total: ~$64.1M**

### Constructors (2)

| # | Constructor | Price | Rationale |
|---|------------|-------|-----------|
| 1 | **Ferrari** | ~$25.0M | TRIPLE FERRARI EXPOSURE. Both Leclerc + Hamilton score for this constructor. If Ferrari 1-2: constructor alone scores 55-65 pts. |
| 2 | **Racing Bulls** | $6.3M | Lindblad synergy. Budget pick with upside if Lindblad's practice pace is real. |

**Constructor total: ~$31.3M**

### Team Total: ~$95.4M (buffer: ~$4.6M)

### 2x Driver: Charles Leclerc (standard captain pick, not a chip)

---

## Scoring Projections

### Best Case: Ferrari 1-2 (Leclerc wins)

| Pick | Calculation | Fantasy Pts |
|------|-----------|-------------|
| Leclerc (2x driver) | (25 race + 5 quali + 3 overtakes + FL) x 2 | ~70-80 |
| Hamilton | 18 race + 5 quali + 3 overtakes | ~26-30 |
| Ferrari (C) | Both drivers race + quali + pit stops | ~55-65 |
| Bearman | Race finish + overtakes | ~10-14 |
| Lindblad | Race finish + overtakes | ~4-8 |
| Bottas | Overtaking from grid penalty | ~2-6 |
| Racing Bulls (C) | Lindblad + Lawson combined | ~8-12 |
| **TOTAL** | | **~175-215** |

### Worst Case: Leclerc DNF + Hamilton out of points

| Pick | Calculation | Fantasy Pts |
|------|-----------|-------------|
| Leclerc (2x driver) | -20 x 2 | **-40** |
| Hamilton | DNF: -20 | **-20** |
| Ferrari (C) | Both drivers out | ~-10 |
| Others | Normal scoring | ~15-25 |
| **TOTAL** | | **~-55 to -25** |

**Swing range: -55 to +215 = 270 point spread.** That's HIGH RISK, HIGH REWARD.

---

## Risk Matrix

| Scenario | Probability | Team #3 Score | Team #1 Score | Edge |
|----------|------------|--------------|--------------|------|
| Ferrari 1-2 | 15% | ~190 | ~120 | +70 |
| Ferrari 1-3 | 20% | ~155 | ~110 | +45 |
| Ferrari top 5 both | 30% | ~130 | ~105 | +25 |
| Ferrari mixed (1 good, 1 bad) | 20% | ~80 | ~95 | -15 |
| Ferrari disaster (DNFs) | 15% | ~-30 | ~70 | -100 |

---

## Why No Mercedes or McLaren

**Deliberate choice.** This team is a CONCENTRATED BET on Ferrari. Diversification is for Team #1 (safe). Team #3 says:

- Mercedes had best testing race pace → but this is Albert Park, not Bahrain
- McLaren Piastri topped FP2 → but testing showed them "a little bit off"
- Ferrari had fastest single laps AND both drivers consistently top 5 → strongest overall signal

If you wanted diversification, play Team #1. Team #3 is for when you believe the data.

---

## Comparison Across All 3 Teams

| | Team #1 (Safe) | Team #2 (Creative) | Team #3 (YOLO) |
|-|---------------|-------------------|----------------|
| Premium drivers | Leclerc + Antonelli | Hadjar (mid) | Leclerc + Hamilton |
| Constructors | Ferrari + Haas | McLaren + Ferrari | Ferrari + Racing Bulls |
| Chip | Save all | Save all | Save all |
| Ferrari picks | 2 (driver + C) | 1 (C only) | **3 (2 drivers + C)** |
| Mercedes picks | 1 (Antonelli) | 0 | **0** |
| McLaren picks | 0 | 1 (C) | **0** |
| Expected range | 90-140 | 80-130 | **-55 to 215** |
| Strategy | Diversified PPM | Constructor-heavy | **Concentrated bet** |
| Cost | $97.4M | $95.1M | $95.4M |

---

## Before Locking In

1. [ ] Verify Ferrari constructor price on fantasy.formula1.com
2. [ ] Confirm Bottas grid penalty is reflected (5 places)
3. [ ] Confirm total within $100M
4. [ ] Lock team before qualifying (Saturday March 7)
5. [ ] Accept that this team could finish last OR first in your league this week
