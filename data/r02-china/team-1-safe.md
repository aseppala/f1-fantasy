# 2026 Chinese GP - Team 1: "Safe" (Round 2)

**Circuit:** Shanghai International Circuit
**Dates:** March 13-15, 2026
**Format:** Sprint Weekend (FP1 > SQ > Sprint > Qualifying > Race)
**Weather:** Dry all weekend. 15-19C race day, light wind. No weather factors.
**Deadline:** Before Sprint Qualifying, Friday March 13

---

## Chip: LIMITLESS

The Limitless chip removes budget + transfer limits for one weekend. Sprint weekend maximizes its value (~30-50% more points). This is the clearest Limitless opportunity we'll get — Mercedes dominance makes the optimal team obvious.

**Why use it now:**
1. Sprint weekend = ~30-50% more points available (sprint + race)
2. Mercedes are dominant — loading up on all premium drivers maximizes the gap
3. Only 6 sprint weekends all season; this is the first one
4. Clear pecking order makes the optimal Limitless team obvious — less guesswork
5. The official F1 Fantasy strategist recommends Limitless this weekend
6. Your R01 team doesn't include Russell (the clear #1) — Limitless fixes this without using transfers

---

## Team

### Drivers

| # | Driver | Team | Price | Exp. Pts | PPM | Rationale |
|---|--------|------|-------|----------|-----|-----------|
| 1 | **George Russell** (2x) | Mercedes | $27.7M | 30.4 (x2=60.8) | 1.10 | Won R01 by 3s. FP1 P1. SQ P1 by 0.29s. Untouchable this weekend. |
| 2 | **Kimi Antonelli** | Mercedes | $23.5M | 26.5 | 1.13 | FP1 P2, SQ P2. Highest PPM of any premium driver. R01 P2. Mercedes lock. |
| 3 | **Lando Norris** | McLaren | $27.1M | 22.2 | 0.82 | FP1 P3, SQ P3. McLaren clearly 2nd-fastest team. |
| 4 | **Oscar Piastri** | McLaren | $25.2M | 22.4 | 0.89 | FP1 P4, SQ P5. Races stronger than he qualifies. Completes McLaren stack. |
| 5 | **Lewis Hamilton** | Ferrari | $22.6M | 20.6 | 0.91 | **CHANGED from Leclerc.** SQ P4 — beat Leclerc (P6) by 0.37s. Better PPM AND $0.5M cheaper. |
| | **Drivers total** | | **$126.1M** | | | |

### Constructors

| # | Constructor | Price | Exp. Pts | PPM | Rationale |
|---|------------|-------|----------|-----|-----------|
| 1 | **Mercedes** | $29.6M | 48.5 | 1.64 | Both drivers 1-2 all weekend. 96 fantasy pts in R01. |
| 2 | **McLaren** | $28.8M | 38.2 | 1.33 | Both drivers P3-P5 in SQ. Clear 2nd-best constructor. |
| | **Constructors total** | **$58.4M** | | | |

**Team total: $184.5M** (Limitless ignores budget) | **Sim avg: 239.1 pts** | Std Dev: ±39.0

### 2x Driver: George Russell

---

## SQ Update: Why Hamilton over Leclerc

Sprint Qualifying settled this debate:
- Hamilton SQ P4 (1:32.161) vs Leclerc SQ P6 (1:32.528) — 0.37s gap
- Hamilton has been quicker than Leclerc in 2 of 3 sessions (SQ P4 > P6, FP1 roughly equal)
- Hamilton 20.6 avg pts vs Leclerc 19.0 — sim confirms the switch
- Hamilton is also $0.6M cheaper (irrelevant with Limitless, but still)

---

## Transfer & Chip Tracker

| Field | Value |
|-------|-------|
| **Total budget** | $102.1M ($100M - $96.2M R01 cost = $3.8M bank + $98.3M squad value) |
| **R02 Limitless squad** | $184.5M (Limitless ignores budget) |
| **Free transfers available** | Unlimited (Limitless) |
| **Transfers used R02** | Limitless (unlimited) |
| **Carry-over to R03** | 0 (Limitless resets transfers) |
| **Chip used R02** | **Limitless** |
| **Chips remaining** | 5 (No Negative, Auto Pilot, Final Fix, Wildcard, x3 Boost) |

### Pre-Limitless Squad (reverts to this for R03)

| # | Pick | R01 Price | R02 Price |
|---|------|-----------|-----------|
| 1 | Charles Leclerc | $22.8M | $23.1M |
| 2 | Kimi Antonelli | $23.2M | $23.5M |
| 3 | Oliver Bearman | $7.4M | $8.0M |
| 4 | Arvid Lindblad | $6.2M | $6.8M |
| 5 | Valtteri Bottas | $5.9M | $5.3M |
| 6 | Ferrari (C) | $23.3M | $23.6M |
| 7 | Haas (C) | $7.4M | $8.0M |
| | **Total** | **$96.2M** | **$98.3M** |

**R03 starting point:** $98.3M squad + $3.8M bank = $102.1M budget, 2 free transfers (no carry-over from Limitless round).

## Before Locking In

1. [ ] Activate Limitless chip on fantasy.formula1.com
2. [ ] Set Russell as 2x driver
3. [ ] Confirm all 5 premium drivers + 2 premium constructors
4. [ ] Remember: team reverts to pre-Limitless squad for R03
